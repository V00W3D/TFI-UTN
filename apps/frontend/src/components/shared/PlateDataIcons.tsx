    ...
    <span className={className} style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }} aria-hidden="true">
        {Array.from({ length: 5 }, (_, index) => {
          ...
          const gradientId = `${baseId}-star-${index}`;
          const offset = `${fillRatio * 100}%`;

          return (
            <svg key={index} width={size} height={size} viewBox="0 0 24 24">
              <defs>
                <linearGradient id={gradientId} x1="0%" x2="100%" y1="0%" y2="0%">
                  <stop offset={offset} stopColor="#f5b301" />
                  <stop offset={offset} stopColor="#efe2b7" />
                  <stop offset="100%" stopColor="#efe2b7" />
                </linearGradient>
              </defs>
              <path
                d={STAR_PATH}
      ...
      {showValue && (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <strong style={{ fontSize: '0.82rem', lineHeight: 1 }}>{safeValue.toFixed(1)}</strong>
          {reviewCount != null ? (
            <span
              style={{
                fontSize: '0.62rem',
